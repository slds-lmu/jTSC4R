...
public class RotationForest 
  ...
  public void train(String[] params){
    if(params.length>1){
      Instances data = ClassifierTools.loadData(params[0]);
      double[] cd=InstanceTools.findClassDistributions(data);
      for(double d:cd)
        System.out.println(d);
      RotationForest rf= new RotationForest();
      rf.setNumIterations(Integer.parseInt(params[1]));
  ...
  public void train(){
    ...
    System.out.println("please input the path of dataset");
    String path = reader.next(); // Scans the next token of the input as an int.
//once finished
    reader.close();
//      Instances data = ClassifierTools.loadData("/Users/wangyu/Downloads/GunPoint/GunPoint_TRAIN");
      Instances data = ClassifierTools.loadData(path);
      double[] cd=InstanceTools.findClassDistributions(data);
      for(double d:cd)
        System.out.println(d);
      RotationForest rf= new RotationForest();
